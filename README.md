...
本目录为 [**Quectel_Location_Reporter**](https://github.com/ytlzq0228/Quectel_Location_Reporter) 在 **LuatOS** 平台上的移植版本，运行于 **Air780EP + 独立 GPS 模块** 方案。原项目（QuecPython）代码未做改动。
...
```
luatos/
├── main.lua           # 入口，加电运行（PROJECT/VERSION 用于 LuatOS 项目管理与 OTA）
├── config.lua         # 配置读取与写回（仅使用 /config.cfg；支持 get/set/del 原始 key）
├── config.cfg         # 运行时配置（可提供 config.cfg.example 作为模板，复制为 config.cfg 后修改）
├── gnss_reporter.lua  # 主循环：外置 GNSS、LBS、Traccar/APRS；无定位时按 still_interval 上报状态
...
├── lbsLoc2.lua        # 合宙单基站定位（LBS 未配置 lbs_server 时使用，需在协程中调用）
├── air153C_wtd.lua    # 外置看门狗 GPIO28（可选，约 150 秒喂一次）
├── libnet.lua         # 网络阻塞 API 封装（APRS 等可选依赖）
├── fota_update.lua    # OTA 占位
├── pins_air780ep.json # 引脚定义（可选）
└── README.md
```

## 配置说明

若仓库提供 `config.cfg.example`，复制为 `config.cfg` 后按需修改；否则新建或直接编辑 `config.cfg`，部署到设备。
...
- **Traccar**：`traccar_host`、`traccar_port`（留空 `traccar_host` 则不上报 Traccar）；`http_timeout`、`max_backoff` 为 HTTP 超时与重试最大退避秒数。
- **APRS**：`aprs_callsign`、`aprs_ssid`、`aprs_passcode`、`aprs_host`、`aprs_port`、`aprs_interval`（最小 30 秒）、`aprs_message`、`aprs_icon`（单字符）；callsign 留空则不上报 APRS。
- **上报策略**：`moving_interval`、`still_interval`、`still_speed_threshold`。无定位时仍按 `still_interval` 向 Traccar 上报状态（设备 id、时间戳、电量、信号、基站等，无 lat/lon）。
- **刷机**：`flash_gpio=30` 表示 Reload 键（GPIO30）按下时退出程序。
- **看门狗**：`wdt_period` 对应硬件看门狗使能（代码内约 150 秒喂一次）。
- **LBS**：`lbs_server` 留空时使用内置 **lbsLoc2**（合宙单基站定位，约 2 分钟/次限制）；`lbs_interval`、`lbs_timeout` 等可选。
...
1. 将本目录下所有 `.lua` 及 `config.cfg` 部署到设备（Luatools 或 OTA）。**Air780EP 上请将 `config.cfg` 放在与 `main.lua` 同一目录**（常见为 `/luadb/`），程序首次运行时会将其复制到根目录 `/config.cfg` 并仅读写该文件。
2. 设置开机自启执行 `main.lua`（具体方式以合宙/设备说明为准）。
3. 上电后等待网络就绪（NET LED 亮），外置 GPS 供电由 GPIO22 自动打开，串口2（默认 115200）接收 NMEA 后解析并上报 Traccar/APRS；无 GNSS 时若配置了 LBS（如不填 `lbs_server` 使用内置 lbsLoc2）则按 `lbs_interval` 尝试基站定位。
4. 远程指令 SET/DEL 只改写根目录 **`/config.cfg`**。
...
- LBS：未配置 `lbs_server` 时使用内置 **lbsLoc2**（合宙单基站定位）；若使用自定义服务器（如移远 LBS），需在 `gnss_reporter.lua` 的 `get_lbs_location()` 中自行实现 HTTP 请求。
